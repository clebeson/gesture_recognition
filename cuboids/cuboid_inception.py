...
class inception_cuboid:
    ...
    def configure(self):
        ...
        self.tfrecord_input_file = '/notebooks/video_tfrecord/video_{}_{}.tfrecords'.format( "train" if self.param.is_training  else "test", self.param.tfrecord_base_name if self.param.is_training  else tf.param.tfrecord_base_name_test)
        self.checkpoint_dir="/notebooks/gesture/tensores/checkpoints/new_{}_D{}_FTFS-{}_FS-{}_SFS-{}_TKS-{}_KS-{}_pool{}_LR-{:.0E}_drop-{}_B{}".format(self.param.tfrecord_base_name,
    ...
    def model(self):
        ...
        with tf.name_scope('regularized_loss'):
            reg_loss = self.regularize(loss, type = 2, scale=self.param.reg_scale)
