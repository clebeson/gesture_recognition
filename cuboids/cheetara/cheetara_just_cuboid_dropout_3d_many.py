...
class inception_cuboid:
    ...
    def cuboid_model(self, tensor, kernel_size,  num_filters, id, spac_kernel=3):
        ...
        # cuboid_layer = tf.squeeze(tf.transpose(temporal, perm=[0, 4, 2,3,1]))
    ...
    def cuboid_vgg_model(self):
        ...
        print(input)
        cuboids_size = 20
        with tf.name_scope('cuboid_1_1'):
            #cuboid= self.inseptinon_layer(input, self.param.channels, self.param.kernels_size, "1")
            cuboid= self.cuboid_model(input, 7, cuboids_size, 1, 5)
            cuboid = self.maxpool(cuboid)
            # cuboid = tf.transpose(tf.squeeze(cuboid), perm=[0,2,3,1])
            cuboid = self.batch_norm(cuboid, "BN_cuboid_1_1")

        with tf.name_scope('cuboid_1_2'):
            #cuboid= self.inseptinon_layer(input, self.param.channels, self.param.kernels_size, "1")
            cuboid= self.cuboid_model(cuboid, 5, cuboids_size, 1, 3)
            cuboid = self.maxpool(cuboid)
            # cuboid = tf.transpose(tf.squeeze(cuboid), perm=[0,2,3,1])
            cuboid = self.batch_norm(cuboid, "BN_cuboid_1_2")
        
        cuboid = tf.transpose(cuboid, perm=[0,4, 2,3,1])
        cuboid  = self.conv3d(cuboid, 1, 1, 64, [1,1,1,1,1],name="output")
        cuboid = tf.reduce_mean(tf.squeeze(cuboid), axis=[1,2])

        

      
        # with tf.name_scope('cuboid_2_1'):
        #     #cuboid= self.inseptinon_layer(input, self.param.channels, self.param.kernels_size, "1")
        #     cuboid= self.cuboid_model(cuboid, 3, cuboids_size, 1, 3)
        #     #cuboid = self.maxpool(cuboid)
        #     # cuboid = tf.transpose(tf.squeeze(cuboid), perm=[0,2,3,1])
        #     cuboid = self.batch_norm(cuboid, "BN_cuboid_2_1")

        # with tf.name_scope('cuboid_2_2'):
        #     #cuboid= self.inseptinon_layer(input, self.param.channels, self.param.kernels_size, "1")
        #     cuboid= self.cuboid_model(cuboid, 3, cuboids_size, 1, 3)
        #     #cuboid = self.maxpool(cuboid)
        #     # cuboid = tf.transpose(tf.squeeze(cuboid), perm=[0,2,3,1])
        #     cuboid = self.batch_norm(cuboid, "BN_cuboid_2_2")
  
        
        # cuboid = self.maxpool(cuboid, d=2)
...
param.batch_size =  96
param.layer_depth = 64
param.initial_learning_rate = 1e-3
param.keep_dropout = 0.96
