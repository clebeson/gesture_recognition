...
#be437c326e2182494eaab234e35a7c159100647850d6409e
...
class inception_cuboid:
    ...
    def __create_inseption_model(self):
        ...
        print("Max Steps:",self.param.max_steps)
        def formatter(d): 
    ...
    def configure(self):
        ...
        self.checkpoint_dir="/notebooks/cuboids/checkpoints/new_{}_D{}_FTFS-{}_FS-{}_SFS-{}_TKS-{}_KS-{}_pool{}_LR-{:.0E}_drop-{}_B{}".format(self.param.tfrecord_base_name,
    ...
    def inseptinon_layer(self, tensor, channels, kernels_size, id):
        ...
        print("conj", c1, c2, c3,stack)
        output = self.tensor_maxpool(stack, layer_id=id)
        print("output", output)
    ...
    def fc_model(self, input, keep = 1.0):    
        ...
        try:
            if self.param.is_training:
                while not coord.should_stop() or sess.should_stop():    
                        #Train
                        _,step = self.sess.run([self.optimizer, self.global_step])
                        if step >= self.param.max_steps:
                            break
...
param.batch_size = 96
param.keep_dropout = 0.90
param.layer_depth = 20
...
# param.depth = 50
# param.total_train =   1080 #9575
# param.total_test = 15

# param.tfrecord_base_name="last_{}_sampled_ucf1".format(param.depth)
# param.tfrecord_base_name_test="last_{}_sampled_ucf1".format(param.depth)

# param.batch_size = 96 #antes 128
