...
class inception_cuboid:
    ...
    def configure(self):
        ...
        self.checkpoint_dir="/notebooks/cuboids/checkpoints/cub_just_{}_D{}_FTFS-{}_FS-{}_SFS-{}_TKS-{}_KS-{}_pool{}_LR-{:.0E}_drop-{}_B{}".format(self.param.tfrecord_base_name,
    ...
    def cuboid_model(self, tensor, kernel_size,  num_filters, id):
    #     with tf.device('/cpu:0'):
        # if len(tensor.shape) == 5:
    ...
    def cuboid_vgg_model(self):
        ...
        with tf.name_scope('cuboid1'):
            ...
            cuboid= self.cuboid_model(input, 11, 2, 1)
        ...
        with tf.name_scope('cuboid2'):
            cuboid= self.cuboid_model(cuboid, 5, 2, 2)
            cuboid = self.maxpool(tf.expand_dims(cuboid,-1))
            cuboid = tf.transpose(tf.squeeze(cuboid), perm=[0,2,3,1])
        ...
        #     cuboid= self.cuboid_model(cuboid, 2, 2, 1)
        #     cuboid = self.maxpool(tf.expand_dims(cuboid,-1))
        #     cuboid = tf.transpose(tf.squeeze(cuboid), perm=[0,2,3,1])
    ...
    def __load_inputs(self, filename, batch_size):
        ...
        if self.param.is_training:
            image_batch, label_batch = tf.train.shuffle_batch(
                ...
                num_threads = 7, 
                capacity = 40 * self.param.batch_size, 
                min_after_dequeue = 30 * self.param.batch_size)
        else:
            image_batch, label_batch = tf.train.batch(
                [image, label], 
                batch_size = self.param.batch_size, 
                num_threads = 7, 
                capacity = 10 * self.param.batch_size)
...
param.batch_size =  96
param.layer_depth = 2048
param.initial_learning_rate = 3e-3
param.keep_dropout = 0.99
param.epochs = 1200 if param.is_training  else 1
