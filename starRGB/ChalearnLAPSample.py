...
class GestureSample(object):
    ...
    def __init__ (self,fileName):
        ...
        if not os.path.exists(rgbVideoPath):
            raise Exception("Invalid sample file. RGB data is not available: "+rgbVideoPath)

        self.rgb = imageio.get_reader(rgbVideoPath,  'ffmpeg')
        ...
        if not os.path.exists(depthVideoPath):
            raise Exception("Invalid sample file. Depth data is not available: "+depthVideoPath)
        self.depth = imageio.get_reader(depthVideoPath,  'ffmpeg')
        ...
        if not os.path.exists(userVideoPath):
            raise Exception("Invalid sample file. User segmentation data is not available: "+userVideoPath)
        self.user = imageio.get_reader(userVideoPath,  'ffmpeg')
        ...
        if not os.path.exists(sampleDataPath):
            sampleDataPath=os.path.join('/'.join(self.samplePath.split('/')[0:-2]), 'labels',self.seqID + '_data.csv')
            if not os.path.exists(sampleDataPath):
                raise Exception("Invalid sample file. Sample data is not available: "+sampleDataPath)
        self.data=dict()
        ...
        if not os.path.exists(labelsPath):
            labelsPath=os.path.join('/'.join(self.samplePath.split('/')[0:-2]), 'labels',self.seqID + '_labels.csv')
            if not os.path.exists(labelsPath):
    ...
    def getFrame(self,video, frameNum):
        ...
        numFrames = video.get_meta_data()['nframes']
        # Check the given file
    ...
    def getDepth(self, frameNum):
        ...
        depth=depthData.astype(numpy.float32)
