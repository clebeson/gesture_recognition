...
class inception_cuboid:
    ...
    def train_cross_validation(self, dataset):
            ...
            for  learning_rate in learning_rates: 
                for  reg_scale in reg_scales: 
                    ...
                    for  reg_type in reg_types: 
                        ...
                        if reg_scale == 0.0: break
        params = params[27:len(params)//2]
        print("params: ",len(params))
        best_acc = {"acc":[0.0], "prec":[], "rec":[],"fsc":[], "params":[]}
        for batch_size, learning_rate, reg_scale, reg_type, dropout, depth in params:
            ...
            while count_fold < 5:
                ...
                while( step < (self.param.max_steps-1) and not got):
                    ...
                    if  step % iter_per_epoch == 0:
                        ...
                        if count_epocs > 300 or acc >  0.98 or count>9: 
                            got = True
                        if count_epocs% 50 == 0:
            ...
            if np.mean(best_acc["acc"]) < np.mean(accur):
                best_acc["acc"] = accur
                best_acc["prec"] = prec
                best_acc["rec"] = rec
                best_acc["fsc"] = fsc
